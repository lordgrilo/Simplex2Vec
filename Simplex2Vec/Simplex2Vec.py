...
class Simplex2Vec():
    ...
    def _precompute_probabilities(self, bias = "LOexp"):
        ...
        for source in tqdm_notebook(self.hasse.nodes(), desc = "Precomputing probabilities"):
            ...
            if self.PROBABILITIES_KEY not in self.hasse.nodes[source]:
                self.hasse.nodes[source][self.PROBABILITIES_KEY] = dict()
                
            for current_node in self.hasse.neighbors(source):
                # Init probabilities dict 
                if self.PROBABILITIES_KEY not in self.hasse.nodes[current_node]:
                    self.hasse.nodes[current_node][self.PROBABILITIES_KEY] = dict()
                    
                with_return_weights = []
                first_travel_weights = []
                if self.NEIGHBORS_KEY in self.hasse[current_node]:
                    current_neighbors = self.hasse.nodes[current_node][self.NEIGHBORS_KEY]
                else:
                ...
                self.hasse.nodes[current_node][self.PROBABILITIES_KEY][source] = prob
                ...
                if current_node not in first_travel_probababilities_computed:
                    ...
                    prob = prob/np.sum(prob)
                    self.hasse.nodes[current_node][self.FIRST_TRAVEL_KEY] = prob
                    first_travel_probababilities_computed.add(current_node)
                self.hasse.nodes[current_node][self.NEIGHBORS_KEY] = current_neighbors
    ...
    def _hasse_RW_helper(self, input_values):
        ...
        def _RW_iteration(prev_node, current_node, rest_steps):
            if rest_steps > 0:
                neighbors_list = self.hasse.nodes[current_node][self.NEIGHBORS_KEY]
                if None == prev_node:
                    prob = self.hasse.nodes[current_node][self.FIRST_TRAVEL_KEY]
                else:
                    prob = self.hasse.nodes[current_node][self.PROBABILITIES_KEY][prev_node]
...
#             mp_pool.close()
    ...
    def refit(self, epochs=1,**skip_gram_params ):
        self._hasse_RW()
